...
#request_start_list_three = ['0103012c', '0203012c', '0303012c', '0403012c']
request_start_list_four = ['0103008c', '0203008c', '0303008c', '0403008c']
response = ''
request = ''
request_start = ''
generator_parameters = {'Generator number': None, 'Engine rpm' : None, 'Generator Front Bearing Temperature' : None,
                        'Generator Rear Bearing Temperature': None, 'Engine Oil Temperature' : None, 'Battery Voltage' : None, 'Engine Operating Hours' : None, 'Engine Oil Pressure' : None,
                        'Generator Phase A Winding Temperature' : None, 'Generator Phase B Winding Temperature' : None, 'Generator Phase C Winding Temperature' : None}
while True:
...
#               generator_parameters['Generator Total Reactive Energy Exported'] = int(response[26:34], 16) # 2 registers
                ...
                generator_parameters['Battery Voltage'] = round(int(response[250:254], 16) * 0.05, 1) # 1 register
                generator_parameters['Engine rpm'] =  int(int(response[254:258], 16) * 0.125) # 1 register
                generator_parameters['Engine Operating Hours'] = int(int(response[258:266], 16) * 0.05) # 2 registers
                #generator_parameters['Automatic Start/Stop State'] = int(response[266:270], 16) # 1 register
                with open('data.txt', 'a') as data:
                    data.write(str(generator_parameters))
                    data.write('\n')
